...
{
public:
    AOELootWorld() : WorldScript("AOELootWorld", { WORLDHOOK_ON_BEFORE_CONFIG_LOAD }) { }

    void OnBeforeConfigLoad(bool /*reload*/) override
    {
        AOELootConfig::instance()->Load();
    }
};
...
void AOELootPlayer::OnPlayerLogin(Player* player)
{
    ...
    AOELootConfig const& config = *AOELootConfig::instance();
    if (config.enabled && config.messageOnLogin)
...
bool AOELootServer::CanPacketReceive(WorldSession* session, WorldPacket const& packet)
{
    ...
    if (player->GetGroup() && !config.allowInGroup)
        return true;

    float const range = config.range;
    ...
    // Work nearest-first and enforce a hard per-click processing cap. The selected
    // corpse counts toward MaxCorpses, so only MaxCorpses - 1 neighbors are merged.
    size_t const maxNearbyCorpses = config.maxCorpses > 1 ? config.maxCorpses - 1 : 0;
    if (maxNearbyCorpses == 0)
        return true;

    nearbyCorpses.sort([player](Creature const* left, Creature const* right)
        {
            return player->GetExactDistSq(left) < player->GetExactDistSq(right);
        });

    if (nearbyCorpses.size() > maxNearbyCorpses)
    {
        auto firstExcluded = nearbyCorpses.begin();
        std::advance(firstExcluded, maxNearbyCorpses);
        nearbyCorpses.erase(firstExcluded, nearbyCorpses.end());
    }

    // Preserve AzerothCore's native path when there is nothing to merge.
    ...
    processedCreatures.reserve(nearbyCorpses.size());
    regularCandidates.reserve(nearbyCorpses.size() * 4);
    questCandidates.reserve(nearbyCorpses.size());
