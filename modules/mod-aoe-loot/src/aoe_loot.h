...
// Cached configuration. Loot packets read this without repeatedly querying ConfigMgr.
struct AOELootConfig
{
    bool enabled = true;
    bool messageOnLogin = false;
    ...
    void Load()
    {
        ...
        messageOnLogin = sConfigMgr->GetOption<bool>("AOELoot.Message", false);
        allowInGroup = sConfigMgr->GetOption<bool>("AOELoot.Group", true);
        range = sConfigMgr->GetOption<float>("AOELoot.Range", 40.0f);
