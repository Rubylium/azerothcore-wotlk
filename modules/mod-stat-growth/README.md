...
All twelve player ranks of Sinister Strike cost 0 energy and generate 45 energy after each cast. The matching
client `Spell.dbc` override is generated by `localTools/patchSinisterStrike.ps1`; `CleanWOTLK/connect.cmd` launches
the client with `-direct` so its action bar and tooltip use the zero-cost data.

Level 10 Rogues also learn Sanguine Veil, a native 25-Energy self buff with a 15-second duration and 20-second
cooldown. While active, it heals the Rogue for 50% of effective damage dealt by attacks, abilities, poisons and
damage-over-time effects. Its spellbook entry, aura, tooltip and custom icon are shipped in `patch-Z.MPQ`.

Opening, Battle Tempo and Killing Momentum are native player auras with standard buff icons, durations and stack
counts. The client addon does not render a separate Rogue combat HUD or proc overlay.

The custom attacks use native weapon-percent spell effects, so weapon damage, attack power, armor, melee bonuses
and weapon-specific rules continue scaling at every level without requiring artificial spell ranks. Shadow Lunge
deals 150% weapon damage, Riposte deals about 165% weapon damage, and Crimson Sweep deals 175% weapon damage.

Level 18 Rogues learn Crimson Sweep, a 35-Energy, 6-second-cooldown attack. It hits enemies within 8 yards and
applies Crimson Wounds for 30% of the direct hit every 2 seconds for 6 seconds. Each tick restores 3 Energy per
active target and feeds Sanguine Veil healing. Consuming Opening increases direct damage by 50%, extends the bleed
to 10 seconds and grants a combo point.

Combat talents are integrated into the custom rotation:

- Improved Sinister Strike is Vanguard's Rhythm: Quick Cut and Sinister Strike deal 10/20% more damage and grant
  Guarded Rhythm, reducing damage taken by 3/6% for 4 seconds.
- The original active Riposte talent is Riposte Mastery: custom Riposte deals 30% more damage, restores 20 extra
  Energy and reduces damage taken by 10% while its dodge aura is active.
- Throwing Specialization is Crimson Reach: Crimson Sweep gains 2/4 yards and Crimson Wounds deals 15/30% more
  damage.
- Aggression also increases Quick Cut, Shadow Lunge, Riposte and Crimson Sweep damage by 3% per rank.
