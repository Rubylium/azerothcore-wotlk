...
--
-- It wears an ethereal's model (display 20986): hooded, wrapped in energy, and floating rather than standing,
-- which reads as an astral keeper without being a named raid boss. Its model scale is 1.0, the same as a
-- person - Algalon, tried first, is 3.0, three times human height, which in the middle of a capital city is
-- ridiculous. The template is still cloned from a harmless mage trainer, so it has a trainer's flags and
-- stats rather than a raid boss's.
...
-- The ethereal, at its own scale. DisplayScale multiplies the model's own CreatureModelScale, so 1 here
-- leaves it at eye level; anything much above that is what made the last attempt loom over the district.
INSERT INTO `creature_template_model`
    (`CreatureID`, `Idx`, `CreatureDisplayID`, `DisplayScale`, `Probability`, `VerifiedBuild`)
VALUES
    (900102, 0, 20986, 1, 1, NULL);

-- One aura, not two: Spirit Particles, the drifting mote effect a dozen stock NPCs already wear. It applies
-- SPELL_AURA_DUMMY, which does nothing on its own, and never expires (duration index 21), so it cannot touch
-- combat, stats or threat. Arcane Power State was layered on top of this at first and was simply too much on
-- a model that already glows.
INSERT INTO `creature_template_addon`
    (`entry`, `path_id`, `mount`, `bytes1`, `bytes2`, `emote`, `visibilityDistanceType`, `auras`)
VALUES
    (900102, 0, 0, 0, 1, 0, 0, '17327');
